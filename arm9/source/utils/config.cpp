...
Config::Config() {
	if (access(sdFound() ? "sd:/_nds/WildEdit/Settings.json" : "fat:/_nds/WildEdit/Settings.json", F_OK) != 0) {
		this->initialize();
	}

	FILE* file = fopen(sdFound() ? "sd:/_nds/WildEdit/Settings.json" : "fat:/_nds/WildEdit/Settings.json", "r");
