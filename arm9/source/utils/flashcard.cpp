...
bool sdFound(void) {
	if (!sdAccessed) {
		if (access("sd:/", F_OK) == 0) sdRead = true;
		else sdRead = false;
...
bool flashcardFound(void) {
	if (!flashcardAccessed) {
		if (access("fat:/", F_OK) == 0) flashcardRead = true;
		else flashcardRead = false;
...
bool bothSDandFlashcard(void) { return (sdFound() && flashcardFound()); };